...
class Markdown extends Component {
    ...
    renderList(node, key, ordered) {
        ...
        return (
            <View key={'list_' + key} style={styles.list}>
                {this.renderNodes(node.props.children, key, { ordered, start })}
            </View>
        );
    }
    ...
    renderListItem(node, key, index, extras) {
        ...
        const bulletIndex = extras.start + index - 1

        if (Utils.isTextOnly(children)) {
            return (
                <View style={styles.listItem} key={'listItem_' + key}>
                    {this.props.renderListBullet ? this.props.renderListBullet(extras.ordered, bulletIndex) : this.renderListBullet(extras.ordered, bulletIndex)}
